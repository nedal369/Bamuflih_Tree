...
function StatCard({ label, value, icon, color = 'primary' }: { label: string; value: number | string; icon: string; color?: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 animate-fade-in-up">
      <div className={`w-10 h-10 rounded-xl ${colors[color]} flex items-center justify-center mb-3 text-lg`}>{icon}</div>
      ...
      <div className="space-y-3">
        {data.map((item, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-sm text-text-secondary w-24 text-start shrink-0">{item.label}</span>
            <div className="flex-1 bg-surface rounded-full h-6 overflow-hidden">
              <div className="h-full bg-primary rounded-full transition-all duration-1000 ease-out flex items-center justify-end pe-2"
...
export default function StatsPage() {
  ...
  const marriageData = stats.marriageStats?.map(m => {
    const labels: Record<string, string> = { married: 'متزوج', divorced: 'مطلق', widowed: 'أرمل', deceased: 'متوفاة' };
    return { label: labels[m.status] || m.status, value: m.count };
  }) || [];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        ...
        <StatCard label="عدد الأجيال" value={stats.maxGeneration} icon="🌳" color="success" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* City distribution as bar chart too */}
        {stats.cityDistribution && stats.cityDistribution.length > 0 && (
          <BarChart data={stats.cityDistribution.map(c => ({ label: c.city, value: c.count }))} label="التوزيع حسب المدينة" />
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fade-in-up">
          ...
          <div className="flex rounded-full overflow-hidden h-8 mb-2">
            <div className="bg-primary flex items-center justify-center transition-all duration-1000"
            ...
            <div className="bg-danger flex items-center justify-center transition-all duration-1000"
        ...
        {marriageData.length > 0 && (
          <BarChart data={marriageData} label="حالات الزواج" />
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fade-in-up">
          ...
          <div className="flex rounded-full overflow-hidden h-8 mb-2">
            <div className="bg-success flex items-center justify-center transition-all duration-1000"
              style={{ width: `${(stats.livingCount / stats.totalMembers) * 100}%` }}>
              <span className="text-xs font-bold text-white">{stats.livingCount}</span>
            </div>
            <div className="bg-gray-400 flex items-center justify-center transition-all duration-1000"
